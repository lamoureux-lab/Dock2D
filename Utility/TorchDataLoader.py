...
def get_interaction_stream(data_path, shuffle=False, number_of_pairs=None, num_workers=0):
	...
	# sampler = RandomSampler(dataset)
	trainloader = torch.utils.data.DataLoader(dataset, batch_size=1, num_workers=num_workers, shuffle=shuffle)
	return trainloader
...
if __name__=='__main__':
	...
	train_stream = get_interaction_stream(train_datapath, number_of_pairs=number_of_pairs)
	valid_stream = get_interaction_stream(valid_datapath, number_of_pairs=number_of_pairs)
	test_stream = get_interaction_stream(test_datapath, number_of_pairs=number_of_pairs)
